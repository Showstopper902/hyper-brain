...
def run(cmd: List[str], *, input_text: Optional[str] = None, check: bool = True) -> subprocess.CompletedProcess:
...
def dispatch_one_job() -> bool:
    """Your original worker: QUEUED -> RUNNING (one job at a time)."""
...
def backlog_running_unclaimed() -> int:
    """User-defined backlog metric: RUNNING jobs with executor_id IS NULL."""
    with get_conn() as conn:
        with conn.cursor() as cur:
            cur.execute(
                """
...
def ensure_hyperbolic_auth() -> None:
    ...
    # Non-interactive: feed the key on stdin (works even if CLI normally prompts)
    p = run(["hyperbolic", "auth", "status"], check=False)
    if p.returncode == 0 and ("logged" in (p.stdout + p.stderr).lower()):
        return
    run(["hyperbolic", "auth", "set-key"], input_text=HYPERBOLIC_API_KEY + "\n", check=True)
    run(["hyperbolic", "auth", "status"], check=False)
...
def extract_id_and_ssh(instance_obj: Dict[str, Any]) -> Tuple[str, str]:
    ...
    if not ssh_cmd:
        # sometimes SSH string is nested
...
def hyperbolic_rent_vm(gpu_count: int) -> Tuple[str, str]:
    # Rent (no --json flag shown in help, so we rent then discover the new instance by diffing)
    before = {extract_id_and_ssh(x)[0] for x in hyperbolic_instances_json()}
    run(["hyperbolic", "rent", "ondemand", "--instance-type", "virtual-machine", "--gpu-count", str(gpu_count)], check=True)
...
def hyperbolic_terminate(instance_id: str) -> None:
    run(["hyperbolic", "terminate", instance_id], check=False)
...
def build_ssh_command(ssh_command_str: str, key_path: str, remote_cmd: str) -> List[str]:
    ...
    # append the remote command as a single argument
...
def bootstrap_executor_vm(instance_id: str, ssh_command_str: str, executor_id: str) -> None:
    ...
    # Everything is written on the VM exactly like your manual steps.
...
def maybe_scale() -> None:
    ...
    try:
        ...
        # Scale down (conservative)
            ...
            to_remove = have - want
            # remove oldest first, but only if not running anything and grace time passed
            now = time.time()
            for inst in managed:
                if to_remove <= 0:
                    break
                ...
                # if null, treat as old
                grace_ok = True
                if last_used is not None:
                    # use DB age (avoid timezone parsing by asking DB age on-demand)
                    with get_conn() as conn:
                        with conn.cursor() as cur:
...
def main() -> None:
    ...
    while True:
        # 1) Keep dispatching one job at a time (your existing behavior)
        did = dispatch_one_job()
        if not did:
            time.sleep(POLL_SECONDS)

        # 2) Periodically autoscale Hyperbolic
        now = time.time()
