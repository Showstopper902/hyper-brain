...
def get_active_executor_ids(conn: psycopg.Connection, assigned_worker_id: str) -> Set[str]:
    """Return executor IDs currently attached to RUNNING jobs for this worker."""
    with conn.cursor() as cur:
        cur.execute(
            """
...
def autoscaler_loop() -> None:
    ...
    while True:
        try:
            ...
            active_executor_ids = get_active_executor_ids(conn, cfg.assigned_worker_id)
            ...
            LOG.info(
                "backlog=%s inflight=%s active_executor_ids=%s want_pods=%s have_pods=%s",
                backlog,
                inflight,
                sorted(active_executor_ids),
                want,
                have,
            )
            ...
            if have < want:
                ...
                extra = have - want
                idle_pods = [p for p in pods if str(p.get("id") or "") not in active_executor_ids]
                LOG.info(
                    "terminating up to %s idle pod(s); idle_candidates=%s",
                    extra,
                    [str(p.get("id")) for p in idle_pods],
                )

                # Only terminate idle pods. Newest idle pods go first so older idle pods stay warm.
                idle_sorted = sorted(idle_pods, key=pod_uptime_seconds)
                to_kill = idle_sorted[:extra]

                if len(to_kill) < extra:
                    LOG.warning(
                        "wanted to terminate %s pod(s) but only found %s idle pod(s); keeping busy pods alive",
                        extra,
                        len(to_kill),
                    )
                ...
                for p in to_kill:
                    ...
                    if not pid:
                        continue
                    try:
                        LOG.info("terminating idle pod id=%s name=%s", pid, p.get("name"))
